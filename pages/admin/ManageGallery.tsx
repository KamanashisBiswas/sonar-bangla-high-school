...
const ManageGallery: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    if(window.confirm(language === 'bn' ? 'আপনি কি এই ছবিটি মুছে ফেলতে চান?' : 'Are you sure you want to remove this photo?')) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2.5">
            <ImageIcon className="text-emerald-700" size={26} /> 
            {language === 'bn' ? 'ফটো গ্যালারি ম্যানেজমেন্ট' : 'Photo Gallery Management'}
          </h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">
            {language === 'bn' ? 'বিদ্যালয়ের বিভিন্ন অনুষ্ঠান ও ক্যাম্পাসের ফটো যুক্ত ও পরিচালনা করুন' : 'Upload, organize and manage campus events and photo album showcase'}
          </p>
        </div>
        <button 
        ...
        >
          <Plus size={18}/> {language === 'bn' ? 'নতুন ছবি যোগ করুন' : 'Add New Photo'}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 relative" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-6 border-b border-slate-100 pb-3">
              {language === 'bn' ? 'নতুন ছবি আপলোড ফরম' : 'Upload New Photo Form'}
            </h3>
            
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                <label className={labelClass}>{language === 'bn' ? 'ছবির URL লিঙ্ক *' : 'Image URL *'}</label>
              ...
              <div>
                <label className={labelClass}>{language === 'bn' ? 'ছবির ক্যাপশন / বিবরণ *' : 'Caption / Description *'}</label>
                <input required value={newImage.caption} onChange={e => setNewImage({...newImage, caption: e.target.value})} className={inputClass} placeholder={language === 'bn' ? "উদা: বিজ্ঞান মেলা ও উদ্ভাবনী প্রজেক্ট প্রদর্শনী" : "e.g. Science Fair & Innovation Projects"} />
              </div>
              <div>
                <label className={labelClass}>{language === 'bn' ? 'ক্যাটাগরি' : 'Category'}</label>
                <input value={newImage.category} onChange={e => setNewImage({...newImage, category: e.target.value})} className={inputClass} placeholder={language === 'bn' ? "উদা: একাডেমিক / খেলাধুলা / অনুষ্ঠান" : "e.g. Academic / Sports / Events"} />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-6 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs shadow-md cursor-pointer">{language === 'bn' ? 'সংরক্ষণ করুন' : 'Save Photo'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
           ...
           <div key={img.id} className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm group hover:shadow-md transition">
              <div className="relative h-44 overflow-hidden bg-slate-100">
                ...
                <div className="absolute inset-0 bg-slate-900/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                   <button 
                     ...
                     className="bg-rose-600 hover:bg-rose-700 text-white p-2.5 rounded-xl shadow-lg transition cursor-pointer"
              ...
              <div className="p-3.5 bg-white border-t border-slate-100">
                 ...
                 <span className="text-[10px] text-emerald-800 font-bold bg-emerald-50 px-2 py-0.5 rounded-md mt-1 inline-block">
                   {img.category || (language === 'bn' ? 'সাধারণ' : 'General')}
                 </span>
              </div>
           </div>
         ))}
         ...
         >
            ...
            <span className="text-xs font-bold mt-2">
              {language === 'bn' ? 'নতুন ছবি যুক্ত করুন' : 'Add New Photo'}
            </span>
         </div>
      </div>
