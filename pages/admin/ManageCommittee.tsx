...
const ManageCommittee: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    if(window.confirm('আপনি কি এই সদস্যকে তালিকা থেকে মুছে ফেলতে চান?')) deleteCommitteeMember(id);
  };
    ...
    <div className="space-y-8 animate-fade-in text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2.5">
            <Users className="text-emerald-700" size={26} /> পরিচালনা পর্ষদ (কমিটি)
          </h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">ম্যানেজিং কমিটির সদস্যবৃন্দ ও তাদের পদবি পরিচালনা করুন</p>
        </div>
        <button 
        ...
        >
          <Plus size={18}/> নতুন সদস্য যুক্ত করুন
        </button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 relative" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-6 border-b border-slate-100 pb-3">নতুন কমিটির সদস্য যোগ</h3>
            
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                <label className={labelClass}>সদস্যের নাম *</label>
                <input required value={newMember.name} onChange={e => setNewMember({...newMember, name: e.target.value})} className={inputClass} placeholder="উদা: মোঃ শফিকুল ইসলাম" />
              </div>
              <div>
                <label className={labelClass}>পদবি</label>
                <input value={newMember.position} onChange={e => setNewMember({...newMember, position: e.target.value})} className={inputClass} placeholder="উদা: সভাপতি / সহ-সভাপতি" />
              </div>
              <div>
                <label className={labelClass}>সদস্যের ধরন</label>
                <input value={newMember.type} onChange={e => setNewMember({...newMember, type: e.target.value})} className={inputClass} placeholder="উদা: দাতা সদস্য / অভিভাবক সদস্য" />
              </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
           <h3 className="font-bold text-slate-800 text-sm">বর্তমান কমিটির সদস্য তালিকা ({committee.length})</h3>
        </div>
          ...
          <table className="w-full text-left text-xs sm:text-sm">
            <thead>
              <tr className="bg-slate-50 text-slate-600 uppercase font-bold tracking-wider text-[11px] border-b border-slate-200">
                <th className="p-4 sm:p-5">সদস্যের ছবি ও নাম</th>
                <th className="p-4 sm:p-5">পদবি</th>
                <th className="p-4 sm:p-5">সদস্যের ধরন</th>
