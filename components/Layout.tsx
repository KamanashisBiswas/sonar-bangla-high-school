        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5 hover:text-white transition">
              <Phone size={13} className="text-emerald-400" /> {toBanglaNum(settings.contactPhone)}
            </span>
            <span className="flex items-center gap-1.5 hover:text-white transition">
              <Mail size={13} className="text-emerald-400" /> {settings.contactEmail}
            </span>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={toggleMenu}>
          ...
          >
                  ...
                  return (
                    <li key={item.path}>
                      ...
                      >
                        ...
                        <ChevronRight size={16} className={isActive ? 'text-emerald-700' : 'text-slate-400'} />
                      </Link>
                    </li>
                  );
                })}
            ...
            <div className="pt-4 border-t border-slate-100 space-y-2">
              ...
              <p className="text-center text-[11px] text-slate-400 mt-2">{t.topbar.eiin}: {toBanglaNum(settings.eiinCode)}</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-slate-900 font-extrabold text-sm">{language === 'bn' ? 'ভর্তি আবেদন ২০২৫' : 'Admissions Open 2025'}</h4>
                  <p className="text-xs text-slate-600">{language === 'bn' ? 'সকল শ্রেণিতে অনলাইনে আবেদন চলছে' : 'Online applications open for all grades'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-slate-900 font-extrabold text-sm">{language === 'bn' ? 'ফলাফল ও মার্কশিট' : 'Results & Marksheets'}</h4>
                  <p className="text-xs text-slate-600">{language === 'bn' ? 'রোল নম্বর দিয়ে দ্রুত রেজাল্ট যাচাই' : 'Instant marksheet generation'}</p>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 pb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12 mb-12">
            ...
            <div>
              <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider mb-5 pb-2.5 border-b border-slate-200 flex items-center gap-2">
                <BookOpen size={16} className="text-emerald-700" /> {language === 'bn' ? 'একাডেমিক ও সেবা' : 'Academic & Services'}
              </h4>
              <ul className="space-y-2.5 text-xs text-slate-600">
                <li><Link to="/admission" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.topbar.onlineAdmission}</Link></li>
                <li><Link to="/result" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.result.title}</Link></li>
                <li><Link to="/academic" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.academic.routineTitle}</Link></li>
                <li><Link to="/students" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.studentsPage.title}</Link></li>
                <li><Link to="/teachers" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.teachers.title}</Link></li>
                <li><Link to="/downloads" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.downloads.title}</Link></li>
              </ul>
            </div>
            ...
            <div>
              <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider mb-5 pb-2.5 border-b border-slate-200 flex items-center gap-2">
                <ShieldCheck size={16} className="text-emerald-700" /> {language === 'bn' ? 'প্রশাসন ও পরিচিতি' : 'Governance & Info'}
              </h4>
              <ul className="space-y-2.5 text-xs text-slate-600">
                <li><Link to="/about" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.about.title}</Link></li>
                <li><Link to="/administration" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.adminPage.title}</Link></li>
                <li><Link to="/notices" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.notices.title}</Link></li>
                <li><Link to="/gallery" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.gallery.title}</Link></li>
                <li><Link to="/contact" className="hover:text-emerald-700 transition flex items-center gap-2"><ChevronRight size={13} className="text-emerald-600" /> {t.contact.title}</Link></li>
                <li><Link to="/admin-login" className="hover:text-emerald-700 transition flex items-center gap-2 text-emerald-800 font-bold"><Lock size={12} /> {t.topbar.adminLogin}</Link></li>
              </ul>
            </div>
            ...
            <div>
              <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider mb-5 pb-2.5 border-b border-slate-200 flex items-center gap-2">
                <Phone size={16} className="text-emerald-700" /> {t.footer.contactInfo}
              </h4>
              <ul className="space-y-3 text-xs text-slate-600">
                ...
                <li className="p-3 bg-amber-50/80 border border-amber-200/80 rounded-2xl">
                  <div className="flex items-start gap-2 text-amber-950">
                    ...
                    <div>
                      <span className="block font-bold text-xs">{language === 'bn' ? 'অফিস সময়সূচি:' : 'Office Hours:'}</span>
                      <span className="text-[11px] text-amber-900">{language === 'bn' ? 'রবি - বৃহস্পতি: সকাল ৯:০০ - বিকাল ৪:০০' : 'Sun - Thu: 9:00 AM - 4:00 PM'}</span>
                    </div>
                  </div>
                </li>
              </ul>
          ...
          <div className="border-t border-slate-200 pt-7 pb-4">
            <div className="mb-3">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                <Files size={14} className="text-emerald-700" /> {language === 'bn' ? 'প্রয়োজনীয় সরকারি ও শিক্ষা পোর্টাল লিংকসমূহ' : 'Important National & Education Portals'}
              </h4>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 text-xs">
              {QUICK_LINKS.slice(0, 8).map((link, idx) => (
                ...
                >
                  <ExternalLink size={11} className="text-slate-400 group-hover:text-emerald-700 flex-shrink-0" />
          ...
          <div className="border-t border-slate-200 mt-6 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-slate-500">
            <p className="text-center sm:text-left text-slate-600">
              &copy; {toBanglaNum(new Date().getFullYear())} <strong className="text-slate-900 font-bold">{language === 'bn' ? settings.schoolName : 'Sonar Bangla High School'}</strong>। {t.footer.rights}
            </p>
            <div className="flex items-center gap-3">
              <span className="text-[11px] text-slate-600">
                {t.footer.developedBy}: <span className="text-emerald-800 font-bold bg-white border border-slate-200 px-2 py-0.5 rounded-md shadow-xs">PerkTech</span>
              </span>
            </div>
          </div>
